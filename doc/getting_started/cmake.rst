...
* **[Line 23-end]** This is the ``clair-c2py`` specific part, which is triggered if the option ``Update_Python_Bindings`` is set to ``ON``:  
...
.. note::
   ...
   * The ``--generate-depfile`` option is used to generate a dependency file, which ensures that the bindings are regenerated only when necessary.
   * However the tools acts **in the source directory**, producing some additional ``.cxx`` source files.
   * An additionnal ``my_module.wrap.hxx`` file is also generated, only used in multiple module situations.
